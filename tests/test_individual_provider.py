...
def test_taxonomy_returns_dict(fake):
    ...
    assert set(taxonomy.keys()) == {'code', 'classification', 'specialization', 'section', 'grouping', 'display_name'}
...
def test_person_name_by_gender_returns_dict(fake):
    ...
    assert set(person_name.keys()) == {'first_name', 'last_name', 'name_prefix', 'name_suffix', 'type_name'}
...
def test_individual_object_addresses_have_purpose(fake):
    individual = fake.individual_object()
    assert 'mailing_address' in individual and individual['mailing_address']['purpose'] == 'Mailing'
    assert 'location_address' in individual and individual['location_address']['purpose'] == 'LOCATION'
    assert 'main_office_address' in individual and individual['main_office_address']['purpose'] == 'Main Office'
